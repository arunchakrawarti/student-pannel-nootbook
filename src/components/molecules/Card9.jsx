...
//           <h3 className="font-inter font-normal text-[13px] md:text-[14px] leading-tight text-center">
//             {label}
//           </h3>
...
//           </div>
//         )}
//         <h1 className={`text-[18px] md:text-[22px] font-semibold leading-none ${valueColor}`}>
//           {value}
//         </h1>
//         <p className="mt-2 text-[11px] md:text-[13px] font-medium text-gray-600 line-clamp-1">{label}</p>
//       </div>
//     );
//   }
//   if (layout === "row") {
...
//           {!reverseText ? (
//             <>
//               <h1 className={`text-[16px] md:text-[20px] font-medium truncate ${valueColor}`}>{value}</h1>
//               <p className="text-[12px] md:text-[14px] text-gray-600 truncate">{label}</p>
//             </>
//           ) : (
//             <>
//               <p className="text-[12px] md:text-[14px] text-gray-600 truncate">{label}</p>
...
const Card9 = ({
  ...
  const iconWrapperClasses = isFullColorCard
    ...
    : `h-10 w-10 md:h-12 md:w-12 rounded-xl flex items-center justify-center shrink-0 ${colorClass}`;
      ...
      <div className="flex items-center gap-2 cursor-pointer whitespace-nowrap group">
        <span
          className={`text-[13px] md:text-[14px] font-medium transition-colors ${
            isActive
              ? "text-[#16344E] font-bold"
              : "text-gray-600 group-hover:text-black"
          }`}
        >
          {label}
        </span>
        <span className="px-2.5 py-0.5 rounded-md text-[11px] md:text-[12px] font-bold text-white bg-[#16344E]">
          {value}
        </span>
      </div>
    );
  ...
  return (
    ...
    >
      ...
      <div
        className={`
          w-[72px] h-[72px] md:w-[80px] md:h-[80px]
          rounded-[18px]
      ...
      >
        {iconSrc ? (
          ...
          <GoFileDirectory
            className={`w-[36px] h-[36px] ${iconColor}`}
          />
        )}
      </div>
      <div className="text-center mt-4 flex flex-col items-center">
        <h3
          className="
            font-inter font-normal text-[16px] text-gray-700  tracking-[-0.15px] text-center
          "
        >
          {label}
        </h3>
  ...
  if (variant === "stat") {
    return (
      <div
        className={`rounded-xl px-4 py-4 md:px-5 md:py-5 text-center border h-full ${
          colorClass || "bg-white border-gray-200"
        }`}
      >
        {iconSrc && (
          <div className="flex justify-center mb-2">
            <div className="h-8 w-8 md:h-9 md:w-9 flex items-center justify-center rounded-full bg-white/70">
              <Image src={iconSrc} height={18} width={18} alt={label} />
            </div>
          </div>
        )}

        <h1
          className={`text-[18px] md:text-[22px] font-semibold leading-none ${valueColor}`}
        >
          {value}
        </h1>
        <p className="mt-2 text-[12px] md:text-[13px] font-medium text-gray-600 line-clamp-1">
          {label}
        </p>
      </div>
    );
      ...
      <div className={`flex items-center gap-3 ${containerClasses}`}>
        ...
        <div className={`flex-1 min-w-0 ${centerText ? "text-center" : ""}`}>
          {!reverseText ? (
            <>
              <h1
                className={`text-[16px] md:text-[20px] font-medium truncate ${valueColor}`}
              >
                {value}
              </h1>
              <p className="text-[12px] md:text-[14px] text-gray-600 truncate">
                {label}
              </p>
            </>
          ) : (
